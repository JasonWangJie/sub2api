    ...
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div class="card">
          <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ t('admin.settings.features.enterpriseInvoice.title') }}
            </h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
              {{ t('admin.settings.features.enterpriseInvoice.description') }}
            </p>
          </div>
          <div class="space-y-5 p-6">
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {{ t('admin.settings.features.enterpriseInvoice.enabled') }}
                </label>
                <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.settings.features.enterpriseInvoice.enabledHint') }}
                </p>
              </div>
              <Toggle v-model="form.enterprise_invoice_enabled" />
            </div>
          </div>
        </div>
...
async function saveSettings() {
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      enterprise_invoice_enabled: form.enterprise_invoice_enabled,
...
async function disableProvidersByType(type: string) {
  const providerKey = type === "usdt" ? "epusdt" : type;
...
const enabledProviderKeyOptions = computed(() => {
  ...
  return providerKeyOptions.value.filter((opt) => enabled.includes(opt.value) || (opt.value === "epusdt" && enabled.includes("usdt")));
});
