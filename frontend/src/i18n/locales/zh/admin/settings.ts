    ...
    settings: {
      ...
      defaults: {
        ...
        billingChargeMultiplierHint:
          '乘到输入、输出、缓存读取费用明细，再按分组/用户/高峰倍率计算用户扣费，使常规 token 请求满足原始费用 × 计费倍率 = 用户扣费。缓存创建、图片 token、搜索等附加费以及普通生图、异步生图、批量生图和视频结算不应用此系数。默认 1；例如 1.1 表示适用分组的上述原始费用提高 10%。合法范围 (0, 10]。',
      ...
      payment: {
        ...
        field_epusdtApiBaseHint: 'Epusdt 公网基础地址，例如 https://pay.example.com。回调必须使用公网 HTTPS。',
        field_epusdtCurrencyHint: '可选 USDT 或 CNY；新建 Epusdt 默认 USDT，历史未配置币种的实例继续按 CNY 兼容。',
        field_epusdtBonusRateHint: 'USDT/CNY 充值赠送比例，范围 0-100%，最多两位小数。',
        field_epusdtNetworksHint: '逗号分隔网络代码，可用 code=显示别名，例如 tron=TRC20,ethereum=ERC20,bsc；旧的纯代码格式继续兼容。',
