...
USDT 充值使用独立的前台 **USDT充值** 页面，仅创建余额充值订单，不参与订阅购买。Epusdt 实例可选择 `USDT` 或 `CNY` 计价；新建实例默认 `USDT`，缺少币种字段的历史实例兼容为 `CNY`。USDT 模式输入 USDT 并按 Coinbase 的实时 USDT/CNY 汇率折算，CNY 模式输入 CNY；两种模式都可配置充值赠送比例。账户余额仍按系统现有 USD 单位入账。
...
Webhook 地址必须使用公网 HTTPS：`https://your-domain.com/api/v1/payment/webhook/epusdt`。Epusdt 回调使用 JSON GMPay 签名；系统会校验 PID、签名、成功状态、币种和金额。USDT/CNY 汇率由服务端从 Coinbase 获取，正常缓存 60 秒，网络短暂失败时最多使用 10 分钟旧缓存；没有可用汇率时暂停 USDT 充值。Epusdt v1 不提供退款和上游取消能力，因此该实例不支持退款；用户取消只结束本地待支付订单。
