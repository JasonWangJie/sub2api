...
| **Epusdt** | USDT 余额充值 | GMPay 托管收银台，支持 CNY/USDT 计价、网络别名和充值赠送 |
...
USDT 充值使用独立的前台 **USDT充值** 页面，仅创建余额充值订单，不参与订阅购买。Epusdt 实例可选择 `USDT` 或 `CNY` 计价；新建实例默认 `USDT`，缺少币种字段的历史实例兼容为 `CNY`。USDT 模式输入 USDT 并按欧易 C2C 买入价（商家卖 U）实时 USDT/CNY 汇率折算，CNY 模式输入 CNY；两种模式都可配置充值赠送比例。账户余额仍按系统现有 USD 单位入账。
...
| **币种** | `USDT` 或 `CNY`，新实例默认 `USDT` | 是 |
| **充值赠送比例** | `0-100%`，最多两位小数 | 否 |
| **网络列表** | 逗号分隔的网络代码，可使用 `code=显示别名`，例如 `tron=TRC20,ethereum=ERC20,bsc`；纯代码格式继续兼容 | 是 |

Webhook 地址必须使用公网 HTTPS：`https://your-domain.com/api/v1/payment/webhook/epusdt`。Epusdt 回调使用 JSON GMPay 签名；系统会校验 PID、签名、成功状态、币种和金额。USDT/CNY 汇率由服务端从欧易 C2C 买入价获取，正常缓存 60 秒，网络短暂失败时最多使用 10 分钟旧缓存。非有限值、超出 `4-12 CNY/USDT` 的报价，以及相对近期有效报价跳变超过 `5%` 的报价都会被拒绝；没有可用汇率时暂停 USDT 充值。Epusdt v1 不提供退款和上游取消能力，因此该实例不支持退款；用户取消只结束本地待支付订单。

启用多个 Epusdt 实例时，所有实例必须使用相同的计价币种和 `bonusRate`；网络代码和别名可以合并展示。创建订单时服务端会重新取得汇率，订单快照会保存汇率、赠送和 CNY/USD 明细，后续汇率变化不会影响已创建订单。页面中的 CNY 是折算预览，账户实际余额仍按 USD 记账。
