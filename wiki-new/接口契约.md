...
`content` 在通过公开状态复核后：本地 durable 对象直接流式返回；OSS 对象 `307` 到当前签名/公开 URL。响应带 `Referrer-Policy: no-referrer` 与 `X-Content-Type-Options: nosniff`。Cache-Control 为浏览器私有短缓存（本地流式默认 `private, max-age=3600`；OSS 跳转按签名剩余有效期收紧，临近过期用 `no-store`）。**私有图库 / 管理端 `view` 仍为 `private, no-store`，不要与广场 content 混用同一缓存策略。**
