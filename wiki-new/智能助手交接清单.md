...
rg -n "writePublishedPlazaObjectStream|redirectToPublishedPlazaObject|private, no-store" backend\internal\handler
...
backend/internal/handler/image_library_handler.go   # plaza content cache helpers + private view no-store
...
frontend/src/views/user/ImagePlazaView.vue         # lightbox v-show / previewOpen
...
- **已批准广场 `GET /image-plaza/:id/content` 允许 `private, max-age` 浏览器缓存**；私有图库 / 管理端 `view` 必须保持 `private, no-store`。不要把广场缓存策略拷到私有查看路径。
- 广场前端 lightbox 用 `v-show` 保留已加载 `<img>`；同一张图再次打开应能复用本地解码，不应每次强制重拉。
...
rg -n "writePublishedPlazaObjectStream|redirectToPublishedPlazaObject|previewOpen" backend\internal\handler frontend\src\views\user\ImagePlazaView.vue
