...
6. `backend/migrations/189_async_image_result_upload_intents.sql`。
...
```text
这是 JasonWangJie/sub2api Fork，VERSION 保持 0.1.162，当前及后续默认在 main 开发和推送。先读 wiki-new/文档索引.md、当前状态与完成度.md、测试与验收记录.md 和 智能助手交接清单.md，再检查脏工作树。185 是持久异步任务，186 是统一图片对象/图库/审核广场，187 是 SC 上传 admission/幂等/恢复，188 是本机延期投稿，189 是异步结果 PUT intent 与 Outbox claim token。异步 Worker 使用 Redis lease token、PostgreSQL CAS/心跳和固定计费命令；模式只由 Key 当前分组决定，默认私有、公开需审核。worker_concurrency 单实例最大 64 且修改后需重启；参考图有数量/总字节/总像素预算。真实 PostgreSQL 多实例、Docker/testcontainers、三家 OSS、真实上游计费和 go test -race 仍待验证。
```
