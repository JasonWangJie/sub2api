...
本页主要描述 `185_async_image_tasks.sql` 建立的持久异步任务、`187_async_image_upload_reservations.sql` 增加的 SC 上传 admission/恢复层，以及 `189_async_image_result_upload_intents.sql` 增加的结果上传意图和 Outbox 认领所有权。站内图片工作台、统一对象图库、审核广场与本机延期投稿（`186`/`188`）是产品层，详见 [图片工作台.md](图片工作台.md)、[图片图库与对象模型.md](图片图库与对象模型.md) 和 [图片广场审核与迁移.md](图片广场审核与迁移.md)。
...
完整字段和约束见 `backend/migrations/185_async_image_tasks.sql`、`backend/migrations/187_async_image_upload_reservations.sql` 与 `backend/migrations/189_async_image_result_upload_intents.sql`。
