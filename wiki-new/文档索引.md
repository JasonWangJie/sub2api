# Sub2API Fork 二次开发交接 Wiki

本目录记录 `JasonWangJie/sub2api` Fork 的二次开发，不替代原项目 `README*.md`、`docs/` 或 `wiki/`。内容覆盖企业开票，以及持久化异步生图任务中心、图片工作台、服务端个人图库、审核广场、安全迁移与本机延期投稿（审核通过后再同步 OSS）。
...
- 发布版本文件当前是 `backend/cmd/server/VERSION = 0.1.178.14`，本次文档更新不主动升级版本号。
- 本轮开发基线是 `51b083d374decf811ac88f8b0194165db9a8ba79`，基线描述为 `v0.1.162-4-g51b083d37`。
- 本次文档更新前 HEAD 为 `29701211d`，以当前 `git status` 为准。
...
- 企业开票迁移 `228` 至 `231`、用户/管理员页面和接口已存在。拒绝申请保留审计记录和拒绝理由，但对应充值来源可重新申请或历史补录；待处理、已完成和历史标记仍锁定。
...
| [企业开票.md](企业开票.md) | 用户申请、拒绝后重提、管理员处理、历史补录、接口与迁移 |
...
| [图片广场审核与迁移.md](图片广场审核与迁移.md) | 审核广场、举报、content 浏览器缓存、旧广场迁移和维护 Worker |
...
站内用户（异步 / 已归档图库）
  ...
  -> 已批准图片广场（`/content` 允许浏览器私有短缓存；私有 `view` 仍为 no-store）
```
...
17. 视频不在本轮范围。
18. 企业开票被拒绝后必须保留原申请快照和拒绝理由，但不能继续占用充值来源；`PENDING`、`COMPLETED` 和历史开票标记仍禁止重复开票。
...
```text
backend/migrations/228_ZJ_invoice_applications.sql
backend/migrations/229_ZJ_historical_invoice_marks.sql
backend/migrations/230_ZJ_enterprise_invoice_setting.sql
backend/migrations/231_ZJ_allow_rejected_invoice_reapplications.sql
...
frontend/src/api/admin/invoices.ts
