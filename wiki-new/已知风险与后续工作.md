...
`2026-08-14` 续更：已批准图片广场 `/content` 改为浏览器私有短缓存（`private, max-age`，OSS 跳转按签名剩余有效期收紧）；私有图库 `view` 仍 `no-store`；前端 lightbox 用 `v-show` 避免关预览即卸 DOM。定向冒烟已通过，浏览器实机二次打开大图仍为 `PENDING`。
...
已批准广场 `/content` 允许 `private, max-age`，是为了二次打开大图不必重下。私有图库与管理端 `view` 仍必须 `no-store`。OSS 跳转缓存窗口必须短于签名剩余有效期（实现上留 60 秒余量）；临近过期回退 `no-store`。若把 `max-age` 盲目拷到私有路径，或缓存窗口长于签名寿命，会出现越权残留或坏链。
