...
更新时间：`2026-08-30`。本页严格区分各次定向验证、完整门禁和真实外部环境/远端交付。任何结果都只能证明执行时对应的代码。

## 当前结论

```text
企业开票拒绝后重新申请定向验证（2026-08-30）：PASSED
图片广场 content 浏览器缓存冒烟（2026-08-14）：PASSED
...
## 2026-08-30 企业开票拒绝后重新申请验证

目标：被拒绝的申请继续保留审计记录和拒绝理由，但不再锁定充值来源；用户可以重新申请，管理员历史补录也将该来源显示为可用。

覆盖行为：

- 用户端申请记录展示 `REJECTED` 的拒绝理由；
- 用户充值记录重新可选，并可提交新的不可变申请快照；
- 管理员历史补录中，`REJECTED` 不再归类为“已申请”；
- `PENDING`、`COMPLETED` 和 `HISTORICAL_COMPLETED` 继续锁定；
- 迁移 `231_ZJ_allow_rejected_invoice_reapplications.sql` 删除全局唯一约束并建立非唯一查询索引；
- 来源级 PostgreSQL advisory transaction lock 和历史标记唯一约束保持不变。
...
cd frontend
pnpm exec eslint src/views/user/InvoicesView.vue src/views/admin/AdminInvoicesView.vue：PASSED
...
生产构建完成，只有仓库既有的 Vite 混合导入、分块大小和 Browserslist 数据过期提示。此次只把发票定向 Go 测试记为通过；Service 全包中的既有无关失败不计入本功能失败，也不据此声称最新后端全包通过。真实 PostgreSQL 升级数据上的 `228` 至 `231` 迁移和并发竞争仍需部署环境验收。

功能契约见 [企业开票.md](企业开票.md)。

## 2026-08-14 图片广场 content 缓存冒烟

目标：已批准广场图再次打开预览时，浏览器可私有缓存；私有图库 `view` 仍禁止缓存。

代码要点：

- `writePublishedPlazaObjectStream` / `redirectToPublishedPlazaObject`（广场 content）
- 私有路径继续 `writeImageObjectStream` / `redirectToImageObject`（`private, no-store`）
- 前端 `ImagePlazaView` lightbox 使用 `v-show`，关闭不销毁 `<img>`

本轮定向冒烟：

```text
go test ./internal/handler/ ./internal/service/ -run "ImagePlaza|PublishedPlaza|WriteImageObject" -count=1：PASSED
  - handler：含 TestPublishedPlazaObjectAllowsBrowserCache（流式 max-age=3600、签名期内收紧、临近过期 no-store）与既有 WriteImageObjectAccess 契约
  - service：无匹配用例（[no tests to run]）
npm test -- --run src/api/__tests__/imagePlaza.spec.ts src/utils/__tests__/ossThumbnail.spec.ts：PASSED，5 tests
浏览器实机复验 /image-plaza 二次打开大图：PENDING（需已部署含本改动的实例）
```

细则见 [图片广场审核与迁移.md](图片广场审核与迁移.md) 与 [接口契约.md](接口契约.md)。
