...
Fork 自研的图片工作流迁移 `185` 至 `189` 已统一重命名为 `NNN_ZJ_description.sql`。迁移器兼容已部署旧数据库：如果 `schema_migrations` 中只有旧文件名且 checksum 与当前 `_ZJ` 文件完全一致，只登记新文件名别名，不再次执行 SQL；checksum 不一致时拒绝启动。
...
- 嵌入迁移中存在五个 `_ZJ` 文件，且不存在对应旧名文件。

本轮提交前的实际验证结果：

```text
go test ./internal/repository ./migrations -count=1：PASSED，3.981s / 0.591s
go vet ./internal/repository ./migrations：PASSED
go test ./internal/handler ./internal/repository ./internal/service -count=1：PASSED，31.183s / 3.756s / 93.732s
