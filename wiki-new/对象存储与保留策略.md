...
异步生成结果使用另一类 `async_image_result_upload_intents`。Worker 在 PUT 前固定 provider、bucket、object key、MIME、字节和 SHA-256；部分成功时 intent 保留，任务结果清单落库后同事务删除。过期清理先检查异步结果、图库和有效投稿引用，只有无活动引用才删除 OSS。该表由迁移 `189_async_image_result_upload_intents.sql` 创建，也被存储身份切换保护统计。
