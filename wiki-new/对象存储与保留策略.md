...
迁移 `186_ZJ_image_library_and_plaza_moderation.sql` 将生成结果元数据规范化到 `image_storage_objects`。`async_image_results`、`image_library_items` 和有效投稿通过同一对象身份共享字节；新代码不得为异步结果归档重复上传一份 OSS 对象。迁移 `187_ZJ_async_image_upload_reservations.sql` 另外为 SC 输入建立 admission reservation、attempt 与 URL alias 事实表。
