...
更新时间：`2026-08-30`。本页记录的是交接时工作树状态，不是发布公告。
...
| 企业开票基础迁移 | `backend/migrations/228_ZJ_invoice_applications.sql` |
| 历史开票补录迁移 | `backend/migrations/229_ZJ_historical_invoice_marks.sql` |
| 企业开票开关迁移 | `backend/migrations/230_ZJ_enterprise_invoice_setting.sql` |
| 拒绝后重新申请迁移 | `backend/migrations/231_ZJ_allow_rejected_invoice_reapplications.sql` |
| 本次文档更新前 HEAD | `29701211d` |
...
本次文档更新不主动修改 `backend/cmd/server/VERSION`。最终报告不能只写 `0.1.178.14`，必须同时写完整 SHA、`git describe`、推送目标和 CI 结果。
...
| 图片广场 content 浏览器缓存 | 已存在（可能 dirty） | 已批准 `/content` 允许 `private, max-age`；私有 `view` 仍 `no-store`；lightbox `v-show` 保留已解码图 |
...
| 企业开票基础流程 | 已存在 | 功能开关、企业资料快照、充值记录选择、用户申请、管理员完成/拒绝和历史补录 |
| 拒绝后重新申请 | 已存在 | 用户申请记录显示拒绝理由；拒绝来源恢复可选并可创建新申请，旧申请继续保留审计 |
| 历史补录状态修正 | 已存在 | `REJECTED` 来源显示可补录；`PENDING`、`COMPLETED` 和历史标记仍显示已占用 |
...
| 企业开票定向测试 | `PASSED 2026-08-30` | Service 与迁移的 `Invoice|invoice` 定向测试通过；迁移 `231` 的索引语义已覆盖 |
...
| 企业开票前端门禁 | `PASSED 2026-08-30` | 相关 Vue 文件 ESLint、完整 typecheck 和生产 build 通过 |
...
企业开票基础功能位于 `v0.1.178.13`，拒绝后重新申请修正位于 `v0.1.178.14`。当前实现将 `REJECTED` 视为非活跃占用：用户可重新选择来源，管理员历史补录列表也显示为可用；旧申请快照和拒绝理由继续保留。并发保护仍使用来源级 PostgreSQL advisory transaction lock，历史标记唯一性不变。完整规则见 [企业开票.md](企业开票.md)，验证证据见 [测试与验收记录.md](测试与验收记录.md)。
