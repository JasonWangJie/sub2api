...
| `GET /api/v1/image-plaza/:publication_id/content` | 公开状态复核后流式返回或 `307` 到当前 OSS URL；允许短时浏览器私有缓存 |
...
列表项的 `image_url` 固定为站内稳定地址 `/api/v1/image-plaza/{imgpub_*}/content`，不把一次签名结果写进列表响应。提示词默认私有。只有投稿时明确 `share_prompt=true` 才能在公开响应和“一键同款”中返回提示词。

### 内容缓存与前端预览

已批准广场图按 publication id 不可变，且仅对已登录广场访客可见，因此 **content 允许浏览器私有缓存**，与私有图库 `view` 的 `no-store` 刻意区分：

| 路径 | Cache-Control | 说明 |
|---|---|---|
| `GET /api/v1/image-plaza/:id/content`（本地流式） | `private, max-age=3600` | 同一 publication 字节不变 |
| `GET /api/v1/image-plaza/:id/content`（OSS `307`） | `private, max-age=min(3600, 签名剩余秒数-60)`；签名将过期时回退 `private, no-store` | 避免缓存指向已失效签名 URL |
| `GET /api/v1/user/image-library/:id/view` 等私有/管理查看 | `private, no-store` | 私有资产与签名轮换，禁止共享/长期缓存 |

前端 `/image-plaza` lightbox 使用 `v-show` 隐藏，关闭预览不销毁 `<img>`；同一张图再次打开可复用已解码位图。换另一张图时才更换 `src`。实现见 `frontend/src/views/user/ImagePlazaView.vue` 与 `writePublishedPlazaObjectStream` / `redirectToPublishedPlazaObject`。
