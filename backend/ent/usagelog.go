...
type UsageLog struct {
	...
	// System charge multiplier applied to token cost components for this request
	BillingChargeMultiplier float64 `json:"billing_charge_multiplier,omitempty"`
...
func (*UsageLog) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case usagelog.FieldInputCost, usagelog.FieldOutputCost, usagelog.FieldCacheCreationCost, usagelog.FieldCacheReadCost, usagelog.FieldTotalCost, usagelog.FieldActualCost, usagelog.FieldRateMultiplier, usagelog.FieldBillingChargeMultiplier, usagelog.FieldAccountRateMultiplier:
...
func (_m *UsageLog) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case usagelog.FieldBillingChargeMultiplier:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field billing_charge_multiplier", values[i])
			} else if value.Valid {
				_m.BillingChargeMultiplier = value.Float64
			}
...
func (_m *UsageLog) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("billing_charge_multiplier=")
	builder.WriteString(fmt.Sprintf("%v", _m.BillingChargeMultiplier))
	builder.WriteString(", ")
