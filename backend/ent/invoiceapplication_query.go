...
type InvoiceApplicationQuery struct {
	...
	withUser   *UserQuery
	withItems  *InvoiceApplicationItemQuery
...
func (_q *InvoiceApplicationQuery) QueryItems() *InvoiceApplicationItemQuery {
	query := (&InvoiceApplicationItemClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *InvoiceApplicationQuery) WithItems(opts ...func(*InvoiceApplicationItemQuery)) *InvoiceApplicationQuery {
	query := (&InvoiceApplicationItemClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *InvoiceApplicationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*InvoiceApplication, error) {
	...
	if query := _q.withItems; query != nil {
		if err := _q.loadItems(ctx, query, nodes,
			func(n *InvoiceApplication) { n.Edges.Items = []*InvoiceApplicationItem{} },
			func(n *InvoiceApplication, e *InvoiceApplicationItem) { n.Edges.Items = append(n.Edges.Items, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *InvoiceApplicationQuery) loadItems(ctx context.Context, query *InvoiceApplicationItemQuery, nodes []*InvoiceApplication, init func(*InvoiceApplication), assign func(*InvoiceApplication, *InvoiceApplicationItem)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(invoiceapplicationitem.FieldApplicationID)
	}
	query.Where(predicate.InvoiceApplicationItem(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(invoiceapplication.ItemsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
