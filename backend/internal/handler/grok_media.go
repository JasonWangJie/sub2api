...
func (h *OpenAIGatewayHandler) handleGrokMedia(c *gin.Context, endpoint service.GrokMediaEndpoint, requestID string) {
	...
	for {
		...
		if err != nil {
			...
			if errors.As(err, &failoverErr) {
				...
				if failoverErr.ShouldReportAccountScheduleFailure() {
					h.gatewayService.ReportOpenAIAccountScheduleResult(account, grokMediaScheduleModelForRequest(c.Request.Context(), account, routingModel, nil), false, nil)
				}
		...
		h.gatewayService.ReportOpenAIAccountScheduleResult(account, grokMediaScheduleModelForRequest(c.Request.Context(), account, routingModel, result), true, nil)
