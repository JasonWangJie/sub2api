...
func (s *OpenAIGatewayService) handleStreamingResponseWithReasoning(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel, reasoningEffort string) (*openaiStreamingResult, error) {
	...
	structuralSemanticOutputFlushed := false
	...
	completeGuardedEvent := func(queueDrained bool) {
			...
			if shouldFlush {
				if err := flushBuffered(); err != nil {
					...
					if completedSemanticEvent {
						firstSemanticOutputFlushed = true
						structuralSemanticOutputFlushed = !completedClientOutputEvent
					}
		...
		if completedClientOutputEvent && !firstOutputProgressObserved {
			firstOutputScanGuard.Store(false)
	...
	finalizeStream := func() (*openaiStreamingResult, error) {
		...
		if stageFirstOutput && eventInProgress {
			// EOF dispatches a complete residual semantic event even without a
			// trailing blank line. Incomplete downstream-only preambles stay private.
	...
	processSSELine := func(line string, queueDrained bool) {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			// Keep the upstream distinction between a real client-visible output
			// (used for failover and first-output timing) and a structural response
			// event that still deserves its own downstream SSE boundary. Empty
			// output_item/content_part events must not disable failover, but local
			// early-flush behavior exposes those complete boundaries to clients.
			startsClientOutput := forceFlushFailedEvent || openAIStreamDataStartsClientOutput(data, eventType)
			if !semanticOutput && eventType == "response.output_item.added" &&
				strings.TrimSpace(gjson.Get(data, "item.type").String()) == "message" {
				semanticOutput = true
			}
			startsVisibleOutput := openAIStreamDataStartsVisibleOutput(data, eventType)
			startsTTFT := openAIStreamDataStartsTTFT(data, eventType, forceFlushFailedEvent, ttftMode)
			...
			if !clientDisconnected && !failureDelivered && !suppressCurrentEvent {
				...
				if prioritizeFirstVisibleText && firstSemanticOutputFlushed && !firstVisibleTextFlushed && !visibleText {
					// Do not let a whitespace-only delta (or another empty structural
					// event) create an early boundary before the first visible text.
					shouldFlush = false
				}
				if structuralSemanticOutputFlushed && !prioritizeFirstVisibleText &&
					strings.TrimSpace(data) != "[DONE]" && !openAIStreamEventTypeIsTerminal(eventType) {
					// An empty structural item establishes the first boundary, but
					// the following burst should remain batched until its terminal
					// event, matching the pre-existing local streaming contract.
					shouldFlush = false
				}
			...
			// Record first visible output time. Structural progress only disarms
			// the first-output retry guard; it must not skew TTFT.
	...
	sendEvent := func(ev scanEvent) bool {
		// Only the timeout-guarded path backpressures the scanner synchronously.
		// Stage-only streams must keep reading while a test/client Flush blocks;
		// the bounded event queue and first-output stage still cap retained data.
		if guardFirstOutput {
