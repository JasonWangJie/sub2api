...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	cost, selectedBillingModel, err = s.calculateOpenAIRecordUsageCostResolvedWithGate(
		ctx,
		result,
		apiKey,
		billingModels,
	...
	if responseModel := responseModelBillingDeclaration(
		...
		if identified, responseChannelPriced := s.hasIdentifiedOpenAIResponsePricing(ctx, responseModel, apiKey); identified {
			...
			responseCost, responseSelectedModel, responseErr := s.calculateOpenAIRecordUsageCostResolvedWithGate(
			...
			if responseErr == nil && responseModelBillingAdoptable(cost, responseCost, baselineChannelPriced, responseChannelPriced) {
				...
				selectedBillingModel = responseSelectedModel
				cost = responseCost
			}
		}
	}
...
func (s *OpenAIGatewayService) calculateOpenAIRecordUsageCost(
	...
	cost, _, err := s.calculateOpenAIRecordUsageCostResolvedWithGate(
		ctx,
		result,
		apiKey,
		billingModels,
...
func (s *OpenAIGatewayService) calculateOpenAIRecordUsageCostResolved(
	...
	return s.calculateOpenAIRecordUsageCostResolvedWithGate(
		...
		&longContextBillingEnabled, time.Time{},
	)
}

func (s *OpenAIGatewayService) calculateOpenAIRecordUsageCostResolvedWithGate(
	ctx context.Context,
	result *OpenAIForwardResult,
	apiKey *APIKey,
	billingModels []string,
