...
func (s *GeminiMessagesCompatService) ForwardNative(ctx context.Context, c *gin.Context, account *Account, originalModel string, action string, stream bool, body []byte) (*ForwardResult, error) {
	...
	if stream {
		...
		if useUpstreamStream {
			...
			usage = usageObj
			imageCounter = stats.imageCounter
		} else {
...
func collectGeminiSSE(body io.Reader, isOAuth bool) (map[string]any, *ClaudeUsage, *geminiImageOutputCounter, error) {
	...
	return collected, usage, stats.imageCounter, err
}

// geminiSSECollectStats keeps both upstream response-signal metadata and the
// local image-output counter while an SSE response is aggregated.
...
// collectGeminiSSEObserved aggregates SSE chunks and optionally exposes each
// unwrapped data payload to the caller for response-signal detection.
...
func (s *GeminiMessagesCompatService) handleNativeNonStreamingResponse(c *gin.Context, resp *http.Response, isOAuth bool, account *Account, upstreamRequestID string) (*geminiNativeResponseResult, error) {
