...
func (s *OpenAIGatewayService) handleOpenAIUpstreamTransportError(ctx context.Context, c *gin.Context, account *Account, err error, passthrough bool) error {
	if errors.Is(err, errOpenAI429Admission) {
		return &UpstreamFailoverError{StatusCode: http.StatusServiceUnavailable, ResponseBody: []byte(`{"error":{"type":"account_temporarily_unavailable","message":"Account is waiting for quota coordination"}}`)}
	}
